...
public class Address {
    ...
    public Address(String address) {
        ...
        if (bytes.length != 25) {
            throw new KeyException("incorrect length " + bytes.length + ", require 25 bytes");
        }
        if (bytes[0] == 0x00) {
            p2pkh = true;
            ...
            throw new KeyException("incorrect version " + bytes[0] + ", require 0x00 or 0x05");
        }
        byte[] checksum = Sha256.getHash(Sha256.getHash(bytes, 0, 21));
        for (int i = 0; i < 4; i++) {
     ...
     * @throws KeyException         if hash length != 20
     */
    public static Address createFromHash(byte[] hash, boolean p2pkh) {
        if (hash.length != 20) {
            throw new KeyException("incorrect length of the hash, requires 20");
        }
