...
public class HdKeyPair {
     ...
     * @throws KeyException         if created private key is not valid
     */
    ...
    public static HdKeyPair deserialize(String string) {
        ...
        if (serialized.length != 82) {
            throw new KeyException("incorrect length");
        }
        BytesInput bytes = new BytesInput(serialized);
        if (bytes.readIntBE() != 0x0488ADE4) {
            throw new KeyException("incorrect version");
        }
        ...
        if (depth == 0) {
            for (byte aByte : fingerprint) {
                if (aByte != 0) {
                    throw new KeyException("non-zero fingerprint byte");
                }
            }
        }
        int keyNumber = bytes.readIntBE();
        if (depth == 0 && keyNumber != 0) {
            throw new KeyException("non-zero key number");
        }
        ...
        if (bytes.read() != 0) {
            throw new KeyException("incorrect first byte of the key, requires 0x00");
        }
        ...
        for (int i = 0; i < 4; i++) {
            if ((checksum[i] & 0xFF) != bytes.read()) {
                throw new KeyException("incorrect checksum");
            }
        }
        // path
        String path = "m";
     ...
     * @throws UnsupportedOperationException if keyNumber < 0
     * @throws KeyException                  if private key is not valid
     */
     ...
     * @throws KeyException         if path is incorrect or created private key is not valid
     */
    public HdKeyPair generateChild(String path) {
        try {
            ...
            throw new KeyException("path is incorrect " + e.getMessage());
        }
    }
