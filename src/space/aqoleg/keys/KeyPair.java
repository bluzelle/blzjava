...
public class KeyPair {
     ...
     * @throws KeyException         if d <= 0 or d >= n
     */
    public KeyPair(BigInteger d, boolean compressedPublic) {
        if (d.signum() <= 0) {
            throw new KeyException("non-positive private key, requires d > 0");
        } else if (d.compareTo(Ecc.secp256k1.getN()) >= 0) {
            throw new KeyException("private key is too big, requires d < n");
        }
        this.d = d;
    ...
    public static KeyPair decode(String wif) {
        ...
        if (bytes.length != 37 && bytes.length != 38) {
            throw new KeyException("incorrect length " + bytes.length + ", requires 37 or 38 bytes");
        }
        if (bytes[0] != (byte) 0x80) {
            throw new KeyException("incorrect version " + bytes[0] + ", requires 0x80");
        }
        ...
        if (compressed && bytes[33] != 0x01) {
            throw new KeyException("incorrect compressed flag " + bytes[33] + ", requires 0x01");
        }
