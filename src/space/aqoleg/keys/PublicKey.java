...
public class PublicKey {
     ...
     * @throws KeyException         if format is incorrect
     * @throws EccException         if key is incorrect
     */
    public static PublicKey createFromBytes(byte[] bytes) {
        if (bytes.length == 65) {
            if (bytes[0] != 0x04) {
                throw new KeyException("first byte of uncompressed key is incorrect, requires 0x04");
            }
        } else if (bytes.length == 33) {
            if (bytes[0] != 0x02 && bytes[0] != 0x03) {
                throw new KeyException("first byte of compressed key is incorrect, requires 0x02 or 0x03");
            }
        } else {
            throw new KeyException("key length is incorrect, requires 33 or 65 bytes");
        }
